function W = TSS_W_second_stage_modified(theta,inp,inp2)
...
for i=1:N
    ...
    temp2 = [];
    for k=1:K
        temp1 = [temp1; squeeze(sum(sum(G1(i,:,:,k,:) + G1(i+N,:,:,k,:)+G1(i+N+N,:,:,k,:),2),3))];
        temp2 = [temp2; squeeze(sum(sum(G2(i,:,:,k,:) + G2(i+N,:,:,k,:)+G2(i+N+N,:,:,k,:),2),3))];
    end
    temp3 = squeeze(sum(G3(i,:,:) + G3(i+N,:,:)+G3(i+N+N,:,:),2));
